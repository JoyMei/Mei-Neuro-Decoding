...
def DE_PSD(data,stft_para):
    ...
    compute DE and PSD
    --------
    input:  data [n*m]          n electrodes, m time points          40*8064
            stft_para.stftn     frequency domain sampling rate       4
            stft_para.fStart    start frequency of each frequency band  频域的起始
            stft_para.fEnd      end frequency of each frequency band    频域的最后
            stft_para.window    window length of each sample point(seconds) 256
            stft_para.fs        original frequency                   128
